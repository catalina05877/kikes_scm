...
$stmt_detalle = $pdo->prepare("SELECT dv.*, th.tipo, th.presentacion, th.precio_por_cubeta
...
<head>
    <meta charset="UTF-8">
    <title>Editar Venta - Huevos Kikes SCM</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Editar Venta #<?php echo str_pad($venta['id'], 6, '0', STR_PAD_LEFT); ?></h1>
        ...
        <form action="procesar_editar_venta.php" method="POST" id="ventaForm">
            ...
            <div id="productos">
                <?php foreach ($detalles as $index => $detalle): ?>
                    <div class="producto-row">
                        <select name="productos[<?php echo $index; ?>][tipo_huevo_id]" required>
                            <option value="">Seleccionar tipo de huevo...</option>
                            <?php foreach ($tipos_huevos as $tipo): ?>
                                <option value="<?php echo $tipo['id']; ?>" data-precio="<?php echo $tipo['precio_por_cubeta']; ?>" data-stock="<?php echo $tipo['stock_actual'] ?? 0; ?>" <?php echo $tipo['id'] == $detalle['producto_id'] ? 'selected' : ''; ?>>
                        ...
                        <input type="number" name="productos[<?php echo $index; ?>][cantidad]" placeholder="Cantidad de cubetas" min="1" value="<?php echo $detalle['cantidad_cubetas']; ?>" required>
            ...
            <div class="total" id="total">Total: $<?php echo number_format($venta['total'], 0, ',', '.'); ?></div>
    ...
    <script>
        let productCount = <?php echo count($detalles); ?>;
